...
<%
			...
			Vector vtLancamentos =  new Vector();
			
			Vector vtPromocoes =  new Vector();
			
			Vector vtMaisVendidos =  new Vector();

			Vitrine oVitrine = new Vitrine();
									
			//Verifica se existe um resulset para montar vitrine
			if(request.getAttribute("vtVitrineProdutos") == null)
			{
				//Monta vitrine de lançamentos
				vtLancamentos = (Vector) oVitrine.MontarVitrineLancamentos();
				
				//Monta vitrine de Promoções
				vtPromocoes = (Vector) oVitrine.MontarVitrinePromocoes();
				
				//Monta Vitrine de Produtos Mais Vendidos				
				vtMaisVendidos = (Vector) oVitrine.MontarVitrineMaisVendidos();
				
				vtProdutos  = (Vector) oVitrine.MontarVitrineLancamentos();
				
			}
			else
			{
				vtProdutos = (Vector) request.getAttribute("vtVitrineProdutos");
			}
...
<%@page import="ovni.negocio.DetalhesEspecificos"%>
  ...
  <%@page import="ovni.negocio.Avaliacao"%>
  
  <% //MontaVitrine da Pesquisa %>
  <% //if(vtProdutos != null) { %>
		...
		<table style="width:100%">      
		           <%
		            ...
		            <%}%>
	<%//}//fim do if para vtProdutos 
	//else{%>
	<% //Montar vitrine default %>
  ...
  <h1>PROMOÇÕES</h1>
		...
		<table style="width:100%">      
		           <%
		           	int cont2 = 0;
		           	for(int i=0;i<vtPromocoes.size();i++)
		                {
		                	cont2++;
		                
		                %>
		                    <td style="width:25%">                    
		                        <form action="/ovni/VisualizarDetalhesProduto" method="get" name="<%=((Produto)vtPromocoes.get(i)).getCodigo()%>">
		                             <div class="centro">
		                             	
		                             	                               
		                                <div style="width:100%;float:left;position:relative">
		                                	<img src="<%=((Produto)vtPromocoes.get(i)).getURLImagem()%>"/>
		                                </div>
		                                <div class="texto12bold_green" style="clear:both"><%=((Produto)vtPromocoes.get(i)).getTitulo()%></div>
		                                <div class="texto11normal_gray"><%=((Produto)vtPromocoes.get(i)).getAutorartista()%></div>
		                                
		                                <%if(((Produto)vtPromocoes.get(i)).getValorPromocional() == 0) { %>
		                                
		                                <div class="texto11bold_red">R$ <%=((Produto)vtPromocoes.get(i)).getValor() %></div>
		                                <%}else{ %>
		                                 <div class="texto11normal_gray">de R$ <strike> <%=((Produto)vtPromocoes.get(i)).getValor() %></strike></div>
		                                 <div class="texto11bold_red">R$ <%=((Produto)vtPromocoes.get(i)).getValorPromocional() %></div>
		                                 <%} %>
		                                
		                                <input type="hidden" name="Codigo_Produto" value="<%=((Produto)vtPromocoes.get(i)).getCodigo()%>">
		                                <div><input onmouseover="Tip('<b>Comprar <%=((Produto)vtPromocoes.get(i)).getTitulo()%> </b>', WIDTH, 180, FADEIN, 400, FADEOUT, 400,BGCOLOR,'#FFFFFF',BORDERCOLOR,'#516A1B',FONTCOLOR,'#516A1B',OPACITY,78)" onmouseout="UnTip()" type="image" onClick="this.form.submit()" src="/ovni/imgs/buttons/btnComprar.gif"/>
